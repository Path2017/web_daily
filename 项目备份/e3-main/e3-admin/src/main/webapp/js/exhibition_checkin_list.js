...
$(function(){
  ...
  utils.sendajax (option,exhibitionauditList)
  /**
   * 初始化列表结束
   */
  // 点击检索
  ...
  $('.pagination-fa').on('click','.pagination li',function(){
    let page = $(this).find('a').attr('data-page')
   ...
   * 展览公司入驻审核列表
   */
  function exhibitionauditList(data) {
    ...
    let str = `<tr>
                ...
                <td>${time}</td>
                <td>
                  <a class="btn btn-primary" href="exhibition_checkin_info.html?id=${data.id}">
